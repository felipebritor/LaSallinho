...
public class GameScreen implements Screen {
	 ...
	 * @param lsBloco
	 */
	private void enquadrarCamera(Rectangle lsBloco) {
		if (lsBloco.y >= Constantes.ALTURA_TELA / 2) {
			camera.position.y = lsBloco.y;
		} else {
			camera.position.y = Constantes.ALTURA_TELA / 2;
		}

		if (lsBloco.x >= Constantes.LARGURA_TELA / 2 && lsBloco.x <= Constantes.LARGURA_MAP - Constantes.LARGURA_TELA / 2) {
			camera.position.x = lsBloco.x;
		} else {
			if (lsBloco.x <= (Constantes.LARGURA_MAP/2)) {
