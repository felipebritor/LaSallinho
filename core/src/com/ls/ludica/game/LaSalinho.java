...
public class LaSalinho {
	 ...
	 * @param camadaColisao
	 */
	public void Mover(TiledMapTileLayer camadaColisao) {
		...
		if (Gdx.input.isKeyPressed(Keys.A)) {
			...
			for (int j = (int) (bounds.y + 1) / Constantes.TILESCALE; j <= (int) (bounds.y + bounds.height - 1) / Constantes.TILESCALE; j++) {
				...
				if (camadaColisao.getCell(i, j) != null) {
	 ...
	 */
	public void Teleporte (TiledMapTileLayer camadaTeleporte){
		if(state == State.STANDING){	
			int x = (int) bounds.x/Constantes.TILESCALE;
			int y = (int) bounds.y/Constantes.TILESCALE;
			int id = 0;
			if (camadaTeleporte.getCell(x, y) != null) {
				id = camadaTeleporte.getCell(x, y).getTile().getId();
			}
			
			for (int i = 0; i < Constantes.TILESHORIZONTAL; i++) {
				for (int j = 0; j < Constantes.TILESVERTICAL; j++) {
					if (camadaTeleporte.getCell(i, j) != null) {
						if(i!=x && i!=x-1 && i!=x+1 && j!=y){
							if(camadaTeleporte.getCell(i, j).getTile().getId() == id){
